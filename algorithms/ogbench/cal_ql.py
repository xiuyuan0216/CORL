...
class TrainConfig:
    # Experiment
    device: str = "cuda"
    env: str = "cube-double-play-singletask-task2-v0"  # OpenAI gym environment name
...
def train(config: TrainConfig):
    ...
    mc_returns = get_return_to_go(train_dataset, env, config)
    train_dataset["mc_returns"] = np.array(mc_returns)
    assert len(train_dataset["mc_returns"]) == len(train_dataset["rewards"])
