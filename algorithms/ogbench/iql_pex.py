# file: algorithms/finetune/iql_pex_ogbench.py
#
# PEX-style offline-to-online training on top of a frozen offline-trained IQL base policy.
#
# Compared with the original policy-decorator version:
#   1) remove residual action composition
#   2) add a learnable online policy pi_theta
#   3) during rollout:
#        - propose offline action a_beta from frozen offline policy
#        - propose online action a_theta from learnable online actor
#        - use current online critic to score both
#        - sample one action by softmax(Q / pex_temperature)
#   4) during evaluation:
#        - use ONLY the online policy
#
# Notes:
#   - This keeps the online actor/Q training style close to your current code:
...
# ============================================================
# Offline IQL classes (kept matching your current code style)
# ============================================================
...
class OfflineReplayBuffer:
    def __init__(self, dataset: Dict[str, np.ndarray], device: str = "cpu"):
        ...
        self.dones = torch.tensor(dataset[terminals_key], dtype=torch.float32, device=device).unsqueeze(-1)
...
class TrainConfig:
    ...
    # load offline IQL checkpoint
    iql_checkpoint: str = ""
    strict_iql_load: bool = True
    base_iql_deterministic: bool = True

    # offline IQL architecture params
    ...
    # logging
    project: str = "CORL"
    group: str = "IQL-PEX-ogbench"
    name: str = "IQL-PEX-ogbench"
    log_wandb: bool = True
...
def train(cfg: TrainConfig):
    ...
    if cfg.normalize_reward:
        reward_mod_dict = modify_reward(train_dataset, cfg.env)
    ...
    actor_optimizer = optim.Adam(list(online_actor.parameters()), lr=cfg.policy_lr)
    ...
    offline_rb = OfflineReplayBuffer(train_dataset, device=device)
    ...
    while global_step < cfg.online_steps:
        ...
        for _ in range(cfg.training_freq):
            ...
            online_rb.add(obs, next_obs, action, float(reward), real_done)
        ...
        if global_step % cfg.eval_freq == 0:
            ...
            eval_log = {
                "eval/score_mean": float(eval_scores.mean()),
                "eval/success_rate": float(success_rate),
                "global_step": global_step,
            }

            eval_successes.append(success_rate)
            eval_log["eval/regret"] = float(np.mean(1 - np.array(eval_successes)))

            print(
                "[Eval {}] ".format(global_step)
                + ", ".join([f"{k}={v:.4f}" for k, v in eval_log.items() if isinstance(v, float)])
            )
