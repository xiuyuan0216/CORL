...
class GaussianPolicy(nn.Module):
    ...
    @torch.no_grad()
    def act_batch(self, state: np.ndarray, device: str = "cpu"):
...
def sample_offline_batch(dataset, batch_size, device):
    indices = np.random.choice(len(dataset["observations"]), size=batch_size)
    data = {
        "observations": torch.tensor(dataset['observations'][indices], device=device),
...
def sample_mix_batch(dataset, rb, batch_size, mixing_ratio, device):
    batch_size_offline = int(batch_size * mixing_ratio)
    batch_size_online = batch_size - batch_size_offline
    
    data_offline = sample_offline_batch(dataset, batch_size_offline, device)
    data_online = rb.sample(batch_size_online)
    
    data = {
        "observations": torch.concat([data_offline['observations'], data_online.observations], dim=0),
        "next_observations": torch.concat([data_offline['next_observations'], data_online.next_observations], dim=0),
        "rewards": torch.concat([data_offline['rewards'].unsqueeze(-1), data_online.rewards], dim=0),
        "actions": torch.concat([data_offline['actions'], data_online.actions], dim=0),
...
def train(cfg: TrainConfig):
    ...
    if cfg.critic_input == "res" and cfg.actor_input == "obs":
        ...
        action_payload_dim = action_dim * 2

    rb = DecoratorReplayBuffer(
        state_dim=state_dim,
        action_dim_payload=action_dim,
    ...
    while global_step < cfg.online_steps:
        ...
        for _ in range(num_updates_per_training):
            global_update += 1
            data = sample_mix_batch(dataset, rb, cfg.batch_size, cfg.mixing_ratio, device)
            ...
            if cfg.critic_input != "res" or cfg.actor_input == "obs_base_action":
                ...
                base_actions = iql.actor.act_batch(data['observations'], device=device)
                base_next_actions = iql.actor.act_batch(data['next_observations'], device=device)
