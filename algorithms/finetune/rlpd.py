# RLPD-style code reduced from the provided Cal-QL implementation.
# Main changes from Cal-QL:
#   1) remove calibration / MC return lower bound
#   2) remove all CQL losses and related hyperparameters
...
class TrainConfig:
    ...
    # Optional BC warmup at the start of offline training
    bc_steps: int = 0
    ...
    # RLPD-style offline/online mixing
    mixing_ratio: float = 0.5   # portion from offline buffer during online stage
...
class RLPD:
    ...
    def _policy_loss(
        ...
        if self.total_it <= self.bc_steps:
            log_probs = self.actor.log_prob(observations, dataset_actions)
    ...
    def _q_loss(
        ...
        with torch.no_grad():
            ...
            td_target = rewards.squeeze(-1) + (1.0 - dones.squeeze(-1)) * self.discount * target_q
        ...
        log_dict.update(
            dict(
                ...
                average_target_q=target_q.mean().item(),
            )
        )
...
def train(config: TrainConfig):
    ...
    for t in range(total_iters):
        ...
        if t < config.offline_iterations:
            ...
            batch = [
                torch.vstack(tuple(b)) for b in zip(offline_batch, online_batch)
            ]
